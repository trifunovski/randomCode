(*Team members: Maksim Trifunovski, Narin Luangrath, Irma Mazariego *)

(* concrete syntax
   lam -> x|(lam lam)|\\x.lam

   abstract syntax
   tm-> var|App(tm,tm)|abs(var,tm)

MISSING: (see below where it says 'your code here')
(0) substitution function [var|-->s]term (subst)
(1) Normal order eval_step function (no_step)
(2) Normal Order eval (no_eval)
(3) Call-by-value eval_step function (eval_step)
(4) Call by Value eval function (eval_cbv)
(5) Conversion back and forth between DeBruijn terms and conventional lambda terms (lam2db, db2lam)
...
(****** YOUR CODE HERE ********)
(* substitution: uses make_fresh_var and rename *)
...
let rec subst var s term =
  match term with
   ...
   |TmAbs(str,t) -> if (str=var)
                    then term
                    else let z = make_fresh_var() in TmAbs(z,subst var s (rename str z t))


(***** Call By Value *****)
(* implement one-step evaluation 'eval_step'
 and then the evaluation function 'eval_cbv' itself
eval_step : term -> term = <fun>
*)

let rec is_value_cbv t =
  match t with
    TmAbs(x,t1) -> true
    |_ -> false
...
let rec eval_step t =
  match t with
    |TmApp(t1,t2) -> (match t1 with
                        TmAbs(x,t3) -> if is_value_cbv t2
                                       then subst x t2 t3
                                       else let t2' = eval_step t2 in TmApp(t1, t2')
                        |_ -> let t1' = eval_step t1 in TmApp(t1', t2)
                     )
    |_ -> raise NO_RULE
...
(* is_a_nf
 *  : term -> bool = <fun>
 *    reports whether or not its argument is a normal form.
 *    *)
let rec is_a_nanf t =
  match t with
    TmVar x -> true
    |TmApp(t1,t2) -> is_a_nanf t1 && is_a_nf t2
    |TmAbs(x,t1) -> false

and is_a_nf t =
  match t with
    TmAbs(x,t1) -> is_a_nf t1
    |_ -> is_a_nanf t
...
let rec no_step t =
  match t with
    |TmApp(nanf,t2) when is_a_nanf nanf -> let t2' = no_step t2 in TmApp(nanf, t2')
    |TmApp(na,t2) when is_a_na na -> (let na' = no_step na in TmApp(na',t2))
    |TmApp(TmAbs(x,t3), t2) -> subst x t2 t3
    |TmAbs(x,t1) -> let t1' = no_step t1 in TmAbs(x,t1')
    |_ -> raise NO_RULE


(* and then... *)
let rec no_eval t =
  try let t' = no_step t in
    no_eval t'
  with NO_RULE -> t;;

(* the define a top level function that sets
the counter to 0 first *)
let top_no_eval t =
  x := 0;
  no_eval t;;
...
let rec showdb d =
  ...
  | DbTmAbs(t) -> "\\"^"."^(showdb t)
  | DbTmApp(t1,t2) -> "("^(showdb t1)^" "^(showdb t2)^")";;
...
let rec getVarIndex v ctx count =
  match ctx with
  | [] -> raise NO_RULE
  | (x::xs) -> if v=x then count else getVarIndex v xs count+1;;
...
let rec getVarName ind ctx =
  match ctx with
  | [] -> raise NO_RULE
  | (x::xs) -> if ind=0 then x else getVarName (ind-1) xs;;

(* convert from DeBruijn to conventional lambda terms *)
let rec db2lam ctx tm =
  match tm with
  | DbTmVar x -> TmVar(getVarName (((count ctx) - 1) - x) ctx)
  | DbTmApp(t1,t2) -> TmApp(db2lam ctx t1,db2lam ctx t2)
  | DbTmAbs(t) -> let newX = make_fresh_var() in TmAbs(newX,db2lam (ctx@[newX]) t);;

(**** Your code here ***)
(* convert from conventional lambda terms to debruijn *)
let rec lam2db ctx tm =
  match tm with
  | TmVar x -> DbTmVar((getVarIndex x (rev ctx []) 0))
  | TmApp(t1,t2) -> DbTmApp(lam2db ctx t1, lam2db ctx t2)
  | TmAbs(x,t) -> DbTmAbs(lam2db (ctx@[x]) t);;
