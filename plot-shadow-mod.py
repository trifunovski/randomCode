...
def plot_tor(data, page, direction="bytes_written"):
    ...
    for (d, label, lineformat) in data:
        ...
        for node in d:
            ...
            for tstr in d[node][direction]:
                ...
                tput[t] += mib
                pertput.append(mib)
            relayTput.append(mib)
            relayIndex.append(index)
            index += 1
        ...
        pylab.figure(relayTP.number)
        x = relayIndex
        y = relayTput
        pylab.scatter(x, y, label=label)
    ...
    pylab.ylabel("Throughput (MiB/s)")
    pylab.title("Throughput per relay")
    pylab.legend(loc="lower right")
    page.savefig()
    pylab.close()
    del(relayTP)
